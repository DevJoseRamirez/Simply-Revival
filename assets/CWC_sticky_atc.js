...
(function () {
  ...
  function initStickyATC() {
    ...
    stickyBars.forEach(function (stickyBar) {
      ...
      // ================================
      const stickyPriceCurrent = stickyBar.querySelector(
        "[data-sticky-price-current]",
      );
      const stickyPriceCompare = stickyBar.querySelector(
        "[data-sticky-price-compare]",
      );

      // Find price elements in static button
      const staticButtonParent = staticButton.closest(
        ".cwc-featured-product__add_to_cart, .cwc_product-template__add_to_cart",
      );
      let staticPriceCurrent = null;
      let staticPriceCompare = null;

      if (staticButtonParent) {
        staticPriceCurrent = staticButtonParent.querySelector(
          ".cwc-featured-product__price-current",
        );
        staticPriceCompare = staticButtonParent.querySelector(
          ".cwc-featured-product__price-compare",
        );
      } else {
        // Fallback: look within the button itself
        staticPriceCurrent = staticButton.querySelector(
          ".cwc-featured-product__price-current",
        );
        staticPriceCompare = staticButton.querySelector(
          ".cwc-featured-product__price-compare",
        );
      }
      ...
      if (staticPriceCurrent && stickyPriceCurrent) {
        const priceObserver = new MutationObserver(function () {
          ...
          if (staticPriceCompare && stickyPriceCompare) {
