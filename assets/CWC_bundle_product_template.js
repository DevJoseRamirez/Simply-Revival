...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const sections = document.querySelectorAll(".cwc-bundle-product-template");
...
function initFeaturedProduct(section, sectionId, variants, sellingPlanGroups) {
  ...
  const optionButtons = section.querySelectorAll(
    ".cwc-bundle-product-template__option_button"
  );
  const optionInputs = section.querySelectorAll(
    ".cwc-bundle-product-template__option-input"
  ...
  // 🔹 Force subscription as default for this bundle section
  let defaultSellingPlanId = null;
  if (sellingPlanGroups.length > 0) {
    ...
    if (firstPlan && firstPlan.id) {
      defaultSellingPlanId = firstPlan.id;
    }
  }

  if (finalSellingPlanInput && defaultSellingPlanId) {
    finalSellingPlanInput.value = defaultSellingPlanId;
    console.log("Initialized default selling plan:", defaultSellingPlanId);

    // Keep UI in sync if you still show the checkbox
  ...
  function updateAllButtonPrices(isAutoRefill) {
    ...
    allButtons.forEach((button) => {
      ...
      const priceEl = button.querySelector(
        ".cwc-bundle-product-template__option_button_price"
      );
      const compareEl = button.querySelector(
        ".cwc-bundle-product-template__option_button_compare_price"
      );
      const saveEl = button.querySelector(
        ".cwc-bundle-product-template__option_button_save_perc"
      );
  ...
  function updateVariant(variant = null) {
    ...
    const priceElement =
      currentPriceEl ||
      section.querySelector(".cwc-bundle-product-template__price-current") ||
  ...
  function handleBundleAddToCart() {
    ...
    const sellingPlanId =
      sellingPlanInputEl && sellingPlanInputEl.value
        ? Number(sellingPlanInputEl.value)
        : null;
  ...
  function initFAQBlocks() {
    ...
    const faqItems = section.querySelectorAll(
      ".cwc-bundle-product-template__faq"
    );
